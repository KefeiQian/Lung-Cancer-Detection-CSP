...
def evaluateCAD(seriesUIDs, results_filename, outputDir, allNodules, CADSystemName, maxNumberOfCADMarks=-1,
    ...
    @param outputDir: output-0.05-0.75 directory
    ...
    for seriesuid in seriesUIDs:
        ...
        for noduleAnnot in noduleAnnots:
            # increment the number of nodules-mask
                ...
                if dist < radiusSquared:
                    if (noduleAnnot.state == "Included"):
                        ...
                        if key not in candidates2.keys():
                            print "This is strange: CAD mark %s detected two nodules-mask! Check for overlapping nodule annotations, SeriesUID: %s, nodule Annot ID: %s" % (str(candidate.id), seriesuid, str(noduleAnnot.id))
                        else:
                            del candidates2[key]
                    elif (noduleAnnot.state == "Excluded"): # an excluded nodule
                        if bOtherNodulesAsIrrelevant: #    delete marks on excluded nodules-mask so they don't count as false positives
    ...
    nodOutputfile.write("    Total number of nodules-mask: %d\n" % totalNumberOfNodules)
    nodOutputfile.write("    Ignored candidates on excluded nodules-mask: %d\n" % irrelevantCandidates)
...
def noduleCADEvaluation(annotations_filename,annotations_excluded_filename,seriesuids_filename,results_filename,outputDir):
    ...
    evaluateCAD(seriesUIDs, results_filename, outputDir, allNodules,
                ...
                maxNumberOfCADMarks=500, performBootstrapping=bPerformBootstrapping,
...
if __name__ == '__main__':
    ...
    results_filename = "submission.csv"
    outputDir = "evaluation"
